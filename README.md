...
* **Lazy Full-Text Top3**
  ...
  * 從 8 則新聞挑最相關 Top3，抓全文並抽取 1–2 段關鍵摘錄
  * 讓模型「有證據可讀」而不是只看標題
...
* 僅對最相關 Top3（從標題做輕量 rerank）抓全文，並抽 1–2 段摘錄：
...
* 部分新聞站可能有：
  ...
  * 反爬（403 / Cloudflare）
  * JS 動態渲染（HTML 正文很少）
    導致全文抓取失敗：系統會自動降級為標題級 evidence（仍保留來源可點）。

* 若想更強的全文解析，可導入更進階的抽取器（例如 readability 類），或對特定站點撰寫專用 parser。

---

## License

本專案僅供學術研究與個人學習用途，請勿用於商業用途。
