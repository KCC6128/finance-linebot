...
* **Grounded RAG / 可驗證引用**
  ...
  * RAG context 會列出新聞 [1]..[N]（N ≤ 8）（來源清單仍含 `title/source/date/url`）
...
+------------------------------+
   ...
   |    - [1]..[N] 來源清單（N ≤ 8，含 title/source/date/url）
   |    - Top3 摘錄沿用相同 [編號]
   |
   v
+------------------------------+
...
* `rag.py` 組 context 時，會列出新聞來源清單 [1]..[N]（N ≤ 8），格式如下：
...
* 回覆會輸出：
  ...
  * 立場：偏多 / 中性（觀望） / 偏空
  * 信心：高 / 中 / 低
* 規則採「證據門檻」（只依據本次 context：股價動態、新聞摘要、全文摘錄）：

  * 偏多 / 偏空：需同向證據 >= 2，且來源需盡量「獨立」（不同新聞條目）
    - 若存在全文摘錄：至少 1 點同向證據應來自全文摘錄（高權重）
    - 若全文摘錄 = 0：仍可偏多/偏空，但必須同向證據 >= 2 且來自不同來源，並「明確下調信心、語氣保守」

  * 中性（觀望）：當證據不足（0–1 點）、或正負證據互相矛盾且無法釐清、或缺關鍵數字/官方說法導致無法驗證方向時，立場必須中性

  * 信心（高/中/低）採保守門檻：
    - 有全文摘錄且多來源同向支持 → 才可能到「中/高」
    - 全文摘錄不足、來源少、或證據衝突 → 傾向「低」，必要時回覆「資料不足，無法判斷」


### (2) 股價動態（不使用新聞引用）

* 股價動態只根據 context 的 **[股價資訊]**（FinMind API）輸出
* 本段也在 prompt 中要求不引用使用新聞引用 [1]..[N]（N ≤ 8）

### (3) 引用規則（必須遵守）

* 凡涉及「具體事實（數字、日期、事件、引述）」或「對公司近況的判斷/原因解讀」
  都必須在句尾標註引用編號，例如：**[1]** 或 **[2][5]**
* 引用編號只能使用 context 中實際出現的 [1]..[N]（N ≤ 8）；不得編造不存在的編號。
